...
import { Camera, File, Upload, X, AlertCircle, CheckCircle, Loader2 } from "lucide-react";
// Ensure MEDICAL_CATEGORIES is imported correctly
...
export default function SubirDocumentoMedicoPage() {
  ...
  // --- Camera States and Logic (Keep as is) ---
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files && e.target.files.length > 0) { validateAndAddFiles(Array.from(e.target.files)); e.target.value = ''; } };
  // --- End File Drag & Drop ---

  // --- File Validation ---
  const validateAndAddFiles = (newFiles: File[]) => {
    if (newFiles.length > 1) { setUploadError("Por favor, sube solo un archivo a la vez."); return; }
    if (files.length >= 1) { setUploadError("Ya has seleccionado un archivo. Elimina el actual para subir otro."); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: "", category: "", date: "", expiry_date: "", provider: "", amount: "", currency: "MXN", tags: "", notes: "", patient_name: "", doctor_name: "", specialty: "" });
      toast({ title: "Éxito", description: "Documento médico guardado correctamente." });
      setTimeout(() => router.push("/dashboard/documentos"), 1500);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="archivo" className="space-y-4">
        <TabsList>
          <TabsTrigger value="archivo" className="flex items-center gap-2"><File className="h-4 w-4" /><span>Archivo</span></TabsTrigger>
              ...
              {files.length > 0 && (
                <div className="mt-6 space-y-2">
                  <h4 className="text-sm font-medium">Archivo seleccionado</h4>
                   {files.map((file, index) => ( <div key={index} className="flex items-center justify-between rounded-md border p-3"> <div className="flex items-center gap-3 overflow-hidden"> <File className="h-5 w-5 text-muted-foreground flex-shrink-0" /> <div className="overflow-hidden"> <p className="text-sm font-medium truncate">{file.name}</p> <p className="text-xs text-muted-foreground">{(file.size / 1024 / 1024).toFixed(2)} MB</p> </div> </div> <Button variant="ghost" size="icon" onClick={() => removeFile(index)} className="flex-shrink-0"><X className="h-4 w-4" /></Button> </div> ))}
                </div>
              )}
            </CardContent>
          </Card>
                 ...
                 <div className="space-y-2">
                   ...
                   <Select value={formData.category} onValueChange={(value) => handleSelectChange("category", value)} required>
                     ...
                     <SelectContent>
                       ...
                       {Array.isArray(MEDICAL_CATEGORIES) ? (
