// startup_clonada/app/dashboard/reportes/reportes-client-content.tsx
"use client"
...
import { reportsService, formatFileSize, type DocumentStats, type CategoryCount, type MonthlyCount, type ExpenseData } from "@/lib/reports-service"
...
export function ReportesClientContent({ initialStats, initialCategoryData, initialAnalysis, initialYear }: ReportesClientContentProps) {
  ...
  const [loadingAnalysis, setLoadingAnalysis] = useState(false); // Initial data is already loaded
  const [categoryData, setCategoryData] = useState<CategoryCount[]>(initialCategoryData);
  const [loadingCategoryData, setLoadingCategoryData] = useState(false); // Initial data is already loaded
  const router = useRouter();

  // Effect to re-fetch general stats when selectedYear changes
  useEffect(() => {
    ...
    // Only fetch if selectedYear changes from initial, or if component mounts with a different year logic
    // This will trigger on subsequent year changes after initial render
  ...
  // Effect to re-fetch category data when selectedYear changes
  useEffect(() => {
  ...
  // Effect to re-fetch analysis (if needed, but typically it's a one-time analysis or triggered by specific user action)
  // Keeping it as is, but it runs only once on mount with no dependency on selectedYear.
  // If analysis should change by year, you'd add selectedYear to its dependencies.
  useEffect(() => {
    ...
    // Only fetch if initialAnalysis wasn't provided or needs refreshing (e.g., if there's a refresh button)
    // For now, assuming initialAnalysis from props is sufficient for initial load.
    // If you want to refetch analysis on every year change, add selectedYear to dependencies.
     // fetchAnalysis(); // Uncomment if you want to refetch analysis dynamically
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="general" className="space-y-4" value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="general" className="space-y-6">
          ...
          {/* DocumentAnalysisComponent might need to be adapted if it also fetches data client-side using server actions.
              For now, assume it consumes props or is client-only without direct server action calls.
              If it makes direct server action calls, you might need to pass initial analysis data to it or
              ensure its internal fetch logic is lazy/conditional. */}
