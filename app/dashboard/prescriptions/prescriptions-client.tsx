        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span className="truncate">{prescription.doctor_name || 'Sin especificar'}</span>
            </div>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-sm text-gray-900 dark:text-white">
              Medicamentos ({prescription.prescription_medicines?.length || 0})
            </h4>
          </div>
          <ul className="space-y-1 ml-6">
            ...
            {(prescription.prescription_medicines?.length || 0) > 2 && (
              <li className="text-sm text-gray-500 dark:text-gray-400 italic ml-6">
                +{(prescription.prescription_medicines?.length || 0) - 2} más
              </li>
            )}
          </ul>
        </div>
...
export default function PrescriptionsClient({ prescriptions }: { prescriptions: any[] }) {
  ...
  const filteredPrescriptions = prescriptions.filter((p: any) =>
    ...
    (p.doctor_name?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false)
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-lg bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-gray-600 dark:text-gray-400">Recientes</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">
        ...
        <div className="p-4 rounded-lg bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 dark:text-white">
                {prescriptions.reduce((sum: number, p: any) => sum + (p.prescription_medicines?.length || 0), 0)}
              </p>
            </div>
          </div>
        </div>
