-- INSTRUCCIONES: Ejecutar este SQL en el SQL Editor de Supabase
-- Dashboard de Supabase -> SQL Editor -> Nueva consulta -> Pegar este código -> Run
...
-- Create shared_resources_with_doctor table
...
DROP POLICY IF EXISTS "Patients can manage their shares" ON public.shared_resources_with_doctor;
DROP POLICY IF EXISTS "Doctors can view shares with them" ON public.shared_resources_with_doctor;
...
DO $$
BEGIN
  RAISE NOTICE 'Migration completed successfully! The shared_resources_with_doctor table has been created.';
END $$;
